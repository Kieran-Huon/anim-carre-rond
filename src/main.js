...
const update = () => {
    ...
    if (elapsed - cooldown.scene2ToScene1 > COOLDOWN_LIMIT) {
        ...
        outScene2_up.forEach(cube => {
            scene2.removeCube(cube);
            const newBubble = scene1.addBubble(cube.position.x + scene1.width / 2, 0); // Entrée en haut de la scène 1
            if (newBubble) {
                newBubble.vy = -Math.abs(cube.body.velocity.y) * 100;
            }
        });
    ...
    /** Scene 1 -> Scene 3 (bulles qui tombent vers le bas) */
    if (elapsed - cooldown.scene1ToScene3 > COOLDOWN_LIMIT) {
        ...
        outScene1_down.forEach(bubble => {
            scene1.removeBubble(bubble);
            const newBubble = scene3.addBubble(bubble.x, 0); // Entrée en haut de la scène 3
            if (newBubble) {
    ...
    /** Scene 3 -> Scene 1 (bulles qui tombent vers le haut) */
    if (elapsed - cooldown.scene3ToScene1 > COOLDOWN_LIMIT) {
        ...
        outScene3_down.forEach(bubble => {
            scene3.removeBubble(bubble);
            const newBubble = scene1.addBubble(bubble.x, scene1.height); // Entrée en bas de la scène 1
            if (newBubble) {
                newBubble.vy = -Math.abs(bubble.vy);
            }
        });
