...
export default class SceneGravityCubes extends Scene3D {
    ...
    constructor(id) {
        ...
        this.wallRight = new Wall('blue');
        this.wallBottom = new Wall('red');
        this.wallLeft = new Wall('green'); // <-- Mur gauche ajouté
        this.add(this.wallRight);
        this.add(this.wallLeft); // <-- Ajouter le mur gauche à la scène
        ...
        this.bodies = [
            this.wallRight.body,
            this.wallLeft.body, // <-- Ajouter le corps du mur gauche au moteur Matter.js
            ...this.cubes.map(c => c.body)
        ];
        Composite.add(this.engine.world, this.bodies);
        this.runner = Runner.create();
    ...
    resize() {
        ...
        if (!!this.wallLeft) {
            this.wallLeft.setPosition(-this.width / 2, 0); // <-- Positionner le mur gauche
            this.wallLeft.setSize(THICKNESS, this.height); // <-- Redimensionner le mur gauche
        }
